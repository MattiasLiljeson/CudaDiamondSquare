...
__device__ float createColor( unsigned char *surface, size_t pitch,
	curandState* const localState, int step, int2 a, int2 b, int2 c,
	int2 d, int width, int height, int idx ){
...
__device__ void diamond( unsigned char *surface, size_t pitch,
	...
	pixel[RED]		= createColor( surface, pitch, localState, step, lt, lb, rt, rb, width, height, RED );
	pixel[GREEN]	= createColor( surface, pitch, localState, step, lt, lb, rt, rb, width, height, GREEN );
	pixel[BLUE]		= createColor( surface, pitch, localState, step, lt, lb, rt, rb, width, height, BLUE );
	pixel[ALPHA]	= 1.0f;
}

__device__ void square( unsigned char *surface, size_t pitch,
	curandState* const rngStates, int x, int y, int step, int width, int height ){
	...
	pixel[RED]		= createColor( surface, pitch, localState, step, t, b, l, r, width, height, RED );
	pixel[GREEN]	= createColor( surface, pitch, localState, step, t, b, l, r, width, height, GREEN );
	pixel[BLUE]		= createColor( surface, pitch, localState, step, t, b, l, r, width, height, BLUE );
	pixel[ALPHA]	= 1.0f;
}

__global__ void cuke_createSquares( unsigned char *surface, int width,
	...
	curandState* const rngStates )
{
	...
	if( (x%(step) == 0) && (y%(step) == 0) ){
		...
		if( xMid && yMid ){
			diamond( surface, pitch, rngStates, x, y, step, width, height );
		}		
	}
}

__global__ void cuke_createDiamonds( unsigned char *surface, int width,
	int height, size_t pitch, float t, int step,
	curandState* const rngStates )
{
	int x = blockIdx.x*blockDim.x + threadIdx.x;
	...
	if( (x%(step) == 0) && (y%(step) == 0) ){
		...
		bool yMid = (y+step) % (step*2) == 0;
		if( xMid != yMid ){
			square( surface, pitch, rngStates, x, y, step, width, height );
		}
	}
}
...
void cu_diamondSquare( void *surface, int width, int height, size_t pitch, 
					  float t, unsigned char* p_rngStates )
{
	...
	for( int i=width; i>k; i/=2 ){
		cuke_createSquares<<< grid, block >>>( (unsigned char *)surface,
			width, height, pitch, (float)i, i, (curandState*)p_rngStates );
		gpuErrchk( cudaPeekAtLastError() );
		gpuErrchk( cudaDeviceSynchronize() );
...
unsigned char* cu_initCurand( int width, int height ){
	...
	initRNG<<< grid, block >>>( rngStates, seed );
	gpuErrchk( cudaPeekAtLastError() );
	gpuErrchk( cudaDeviceSynchronize() );
